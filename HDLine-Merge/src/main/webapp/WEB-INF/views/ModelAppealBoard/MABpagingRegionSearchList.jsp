...
<div style="align-content: center;">
   <form method="get" action="/modelboard/list/${region }/search">
...
<!-- 						<option value="nickname">닉네임</option> -->
				...
				<c:if test="${ userClass == 'model'}">
						<button type="button"
							class="btn btn-outline-secondary btn-sm"
							onclick="location.href='/modelboard/writeform';">글쓰기</button>
				</c:if>
				</td>
	...
	<c:choose>
		...
		<c:otherwise>
		 <c:forEach items="${ boardPage.content}" var="list" varStatus="loop"> 
	   <div class="card mb-8" style="max-width: 80%; margin: auto; float: none;">
		  	<div class="row g-0">
			    <div class="col-md-4" style="max-width: 300px;">
			    	<div style="width:200px; height: 200px; overflow: hidden">
			      		<img src="/img/long${loop.index % 16 + 1}.jpg" class="img-fluid img-thumbnail rounded-start" alt="...">
			      	</div>
	   		 	</div>
		    	<div class="col-md-8">
			      	<div class="card-body">
			        	<h5 class="card-title"><a href="/modelboard/detail/${list.MABno }">${list.title} </a></h5>
...
<%-- 			        	<p class="card-text" ><a class="text-muted" href="#" target="_blank">${list.user.no }</a></p> --%>
			        	...
			        	<p class="card-text">
			        		<a href="/modelboard/${list.MABno}/updateform" class="btn btn-light">수정</a>
			        		<a href="/modelboard/${list.MABno}/delete" class="btn btn-danger" id="delete">삭제</a>
...
<%--                         </c:when> --%>
<%--                         <c:otherwise> --%>
<%--                             <li><a href="${pageContext.request.requestURI }?page=${i.index}">${i.index + 1}</a></li> --%>
...
<%--         </c:if> --%>
        <c:if test="${not empty boardPage.content}">
            <ul class="pagination" style="align-self: center;">
                <c:choose>
                    <c:when test="${boardPage.number > 0}">
                        <li><a href="../${region}/search?searchField=${searchField }&keyword=${keyword}&page=0">First</a></li>
                        <li><a href="../${region}/search?searchField=${searchField }&keyword=${keyword}&page=${boardPage.number - 1}">Previous</a></li>
                    </c:when>
                    <c:otherwise>
                        <li class="disabled"><span>First</span></li>
                        <li class="disabled"><span>Previous</span></li>
                ...
                <c:forEach begin="0" end="${boardPage.totalPages - 1}" varStatus="i">
                    <c:choose>
                        ...
                        <c:otherwise>
                            <li><a href="../${region}/search?searchField=${searchField }&keyword=${keyword}&page=${i.index}">${i.index + 1}</a></li>
                        </c:otherwise>
                    </c:choose>
                </c:forEach>
                ...
                <c:choose>
                    <c:when test="${boardPage.number < boardPage.totalPages - 1}">
                        <li><a href="../${region}/search?searchField=${searchField }&keyword=${keyword}&page=${boardPage.number + 1}">Next</a></li>
                        <li><a href="../${region}/search?searchField=${searchField }&keyword=${keyword}&page=${boardPage.totalPages - 1}">Last</a></li>
                    </c:when>
                    <c:otherwise>
                        <li class="disabled"><span>Next</span></li>
                        <li class="disabled"><span>Last</span></li>
